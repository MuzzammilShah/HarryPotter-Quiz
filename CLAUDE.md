...
5. **Certificate PNG had a visible vertical seam/block artifact across the right half of the card, plus a hard opaque-black border instead of a soft glow** — this was found only by inspecting an actual downloaded PNG (not the live page, which looked correct) after the `html2canvas` swap in bug #3. Root cause: `html2canvas` has known rendering bugs with (a) `radial-gradient` backgrounds that use non-trivial size/position keywords (the card used `radial-gradient(120% 100% at 50% 0%, ...)` — it tessellates/tiles these internally and the tile seams show up as visible blocks), and (b) `inset` box-shadow (renders as a solid opaque fill rather than a soft inner shadow, which combined with the border produced the hard black edge). **Fix:** `CertificateCard.jsx`'s card background was changed from `radial-gradient(...)` to a plain `linear-gradient(180deg, ...)`, and `boxShadow` had its `inset ...` term removed, keeping only the outer glow. Both look effectively identical on-screen to the old versions but export cleanly. **Do not reintroduce `radial-gradient` or `inset box-shadow` anywhere inside `CertificateCard.jsx`** (or any node passed to `html2canvas`) without downloading and visually inspecting a real exported PNG first — the live page will look fine either way and won't reveal the bug.
